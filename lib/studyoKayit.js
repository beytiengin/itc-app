...
// STUDYO-VAK-C5
// Baskın VAK kanalını döndür: 'gorsel' | 'isitsel' | 'kinestetik' | null
// GERÇEK ŞEMA (kalibrasyon.js/-kaydet'ten doğrulandı): vak_sonuclari
//   (kullanici_id, gorsel, isitsel, kinestetik, baskin, tarih) — order tarih.
export async function studyoVakKanali(userId) {
  if (!userId) return null;
  const { data } = await supabase
    .from('vak_sonuclari')
    .select('*')
    ...
    .limit(1).maybeSingle();
  if (!data) return null;
  const adaylar = { gorsel: data.gorsel, isitsel: data.isitsel, kinestetik: data.kinestetik };
  let enYuksek = null, max = -Infinity;
  for (const k in adaylar) if (typeof adaylar[k] === 'number' && adaylar[k] > max) { max = adaylar[k]; enYuksek = k; }
  return enYuksek;
}
