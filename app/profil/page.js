...
function YorumSatir({ baslik, ust, body }) {
  return (
    <div style={{ borderLeft: '2px solid color-mix(in srgb, var(--accent) 32%, transparent)', paddingLeft: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
      <span style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.58rem', letterSpacing: '0.28em', color: 'var(--ink-muted)', textTransform: 'uppercase' }}>{baslik}</span>
      {ust && (
        <span style={{ fontFamily: 'Cormorant Garamond, serif', fontStyle: 'italic', fontSize: '1.08rem', color: 'var(--accent)', lineHeight: 1.3 }}>{ust}</span>
      )}
      {body && (
        <p style={{ fontFamily: 'Cormorant Garamond, serif', fontStyle: 'italic', fontSize: '1rem', color: 'var(--ink)', margin: 0, lineHeight: 1.65 }}>{body}</p>
      )}
    </div>
  );
}
