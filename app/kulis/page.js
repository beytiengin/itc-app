    ...
    <main style={{ minHeight: '100vh', backgroundColor: '#0a0a0a', color: '#f0ede8', display: 'flex', flexDirection: 'column' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '2rem 3rem', borderBottom: '1px solid #2a2a2a' }}>
        ...
        <div style={{ display: 'flex', gap: '2rem', alignItems: 'center' }}>
          ...
          <a href="/antrenman/karakter" style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.6rem', letterSpacing: '0.25em', color: '#f0ede8', textTransform: 'uppercase', textDecoration: 'none' }}>← Ana Ekran</a>
        </div>
      </header>
      ...
      <section style={{ flex: 1, padding: '3rem 2rem', maxWidth: '680px', margin: '0 auto', width: '100%', display: 'flex', flexDirection: 'column', gap: '3rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
          ...
          <span style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.6rem', letterSpacing: '0.4em', color: '#c9a96e', textTransform: 'uppercase' }}>Kulis</span>
          <h1 style={{ fontFamily: 'Cormorant Garamond, serif', fontWeight: 300, fontSize: 'clamp(2rem, 5vw, 3.5rem)', color: '#f0ede8', margin: 0 }}>İz Bıraktıkların</h1>
...
function KarakterKarti({ karakter, egzersizSayisi, yansimaSayisi, aktivite, ilerleme, isAcik, setAcik }) {
  ...
  return (
    <div style={{ border: `1px solid ${isAcik ? '#c9a96e' : '#2a2a2a'}`, backgroundColor: isAcik ? '#0f0f0f' : 'transparent', transition: 'all 0.3s ease' }}>
      <button
        onClick={setAcik}
        style={{
          display: 'flex',
        ...
        onMouseEnter={(e) => { if (!isAcik) e.currentTarget.parentElement.style.borderColor = '#c9a96e'; }}
        onMouseLeave={(e) => { if (!isAcik) e.currentTarget.parentElement.style.borderColor = '#2a2a2a'; }}
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
            ...
            <span style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.65rem', color: '#c9a96e', letterSpacing: '0.1em' }}>{karakter.yazar}</span>
          </div>
          {aktivite && (
            <span style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.6rem', color: '#666', letterSpacing: '0.1em', textTransform: 'uppercase', flexShrink: 0, fontStyle: 'italic' }}>
              {aktivite}
            </span>
          )}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem', marginTop: '0.5rem' }}>
          <IlerlemeSatiri etiket="Egzersizler" deger={egzersizSayisi} toplam={egzersizToplam} renk="#c9a96e" />
          <IlerlemeSatiri etiket="Boşluklar" deger={yansimaSayisi} toplam={boslukToplam} renk="#6a9b6a" />
        </div>

        <span style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.6rem', color: '#666', letterSpacing: '0.2em', textTransform: 'uppercase', alignSelf: 'flex-end', marginTop: '0.3rem' }}>
          {isAcik ? 'Kapat ▴' : 'Detay ▾'}
        </span>
      </button>

      {isAcik && (
        <div style={{ borderTop: '1px solid #2a2a2a', padding: '1.8rem 2rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>

          {tamamlananEgzersizler.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
              <span style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.55rem', letterSpacing: '0.3em', color: '#c9a96e', textTransform: 'uppercase' }}>
                Tamamlanan Egzersizler
              </span>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {tamamlananEgzersizler.map((e) => (
                  <div key={e.id} style={{ display: 'flex', gap: '0.7rem', alignItems: 'center', padding: '0.7rem 1rem', borderLeft: '2px solid #c9a96e44' }}>
                    <span style={{ color: '#6a9b6a', fontSize: '0.75rem' }}>✓</span>
                    <span style={{ flex: 1, fontFamily: 'Cormorant Garamond, serif', fontStyle: 'italic', fontSize: '0.95rem', color: '#ddd', lineHeight: 1.5 }}>{e.baslik}</span>
                    <span style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.55rem', color: '#666', letterSpacing: '0.15em', textTransform: 'uppercase' }}>{e.seviye}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {yazilanBosluklar.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
              <span style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.55rem', letterSpacing: '0.3em', color: '#6a9b6a', textTransform: 'uppercase' }}>
                Yazılan Boşluklar
              </span>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
                {yazilanBosluklar.map(({ boslukId, metin, bosluk }) => {
                  ...
                  return (
                    <div key={boslukId} style={{ padding: '0.9rem 1.1rem', borderLeft: '2px solid #6a9b6a55', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                      ...
                      <p style={{ fontFamily: 'Cormorant Garamond, serif', fontStyle: 'italic', fontSize: '0.88rem', color: '#aaa', lineHeight: 1.7, margin: 0 }}>
                        "{onizleme}"
                      </p>
                    </div>
                  );
...
function IlerlemeSatiri({ etiket, deger, toplam, renk }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
      <span style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.65rem', color: '#aaa', width: '90px', flexShrink: 0, letterSpacing: '0.05em' }}>{etiket}</span>
      ...
      <span style={{ fontFamily: 'Jost, sans-serif', fontWeight: 300, fontSize: '0.7rem', color: '#ccc', width: '50px', textAlign: 'right', flexShrink: 0, letterSpacing: '0.05em' }}>
        {deger}/{toplam}
      </span>
    </div>
  );
...
function EmptyKalibrasyon() {
  return (
    <div style={{ border: '1px solid #2a2a2a', padding: '2.5rem 2rem', display: 'flex', flexDirection: 'column', gap: '1.2rem', alignItems: 'center', textAlign: 'center' }}>
      <span style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.6rem', letterSpacing: '0.4em', color: '#c9a96e', textTransform: 'uppercase' }}>Önce Kalibrasyon</span>
      <p style={{ fontFamily: 'Cormorant Garamond, serif', fontStyle: 'italic', fontSize: '1.1rem', color: '#ddd', lineHeight: 1.7, margin: 0, maxWidth: '420px' }}>
      ...
      <a
        href="/kalibrasyon"
        style={{ marginTop: '0.5rem', padding: '0.9rem 2rem', backgroundColor: '#c9a96e', color: '#0a0a0a', textDecoration: 'none', fontFamily: 'Jost, sans-serif', fontWeight: 300, fontSize: '0.7rem', letterSpacing: '0.25em', textTransform: 'uppercase', transition: 'all 0.3s ease' }}
...
function EmptyMacbethBasla() {
  return (
    <div style={{ border: '1px solid #2a2a2a', padding: '2.5rem 2rem', display: 'flex', flexDirection: 'column', gap: '1.2rem', alignItems: 'center', textAlign: 'center' }}>
      <span style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.6rem', letterSpacing: '0.4em', color: '#c9a96e', textTransform: 'uppercase' }}>Henüz İz Yok</span>
      <p style={{ fontFamily: 'Cormorant Garamond, serif', fontStyle: 'italic', fontSize: '1.1rem', color: '#ddd', lineHeight: 1.7, margin: 0, maxWidth: '420px' }}>
