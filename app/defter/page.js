...
export default function KulisSayfasi() {
  const { dil } = useDil();
  // chrome-i18n.kulis (sayfa sözlüğü) + KARARLAR_METIN (yeni bölüm metinleri)
  const t = useMemo(
    () => ({ ...ceviri(chromeI18n, dil).kulis, ...(KARARLAR_METIN[dil] || KARARLAR_METIN.tr) }),
    [dil]
  );

  // Çok-dilli mimari: Hamlet + Willy + Macbeth + Biff karakter dramatik
  // içeriği dil katmanıyla merge edilir. Biff DE Sprint 5'te eklenir
  // (TR fallback şimdilik korunur).
  ...
  const [kararlar, setKararlar] = useState({ yorumlar: [], muhurler: [], bos: true });
...
// ─── DETAY: BÖLÜMLER ────────────────────────────────────────────────────────
// Sıra: Nerede Kaldın · Verdiğin Kararlar (yeni) · Yazdıkların (Biriken) ·
