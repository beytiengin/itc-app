// app/antrenman/karakter/macbeth/el-yazmasi/page.js
...
//   1. Başlık (Modül II · Macbeth · El yazması)
...
// - Substitution yok: tüm sorgular karakter-merkezli ("Macbeth bunu yaşarken…").
...
import macbethI18n, { macbethIcerik } from '../../../../../data/macbeth-i18n';
...
// MACBETH SPRINT 3: kayıt anları + hassas/provisional setler boş bırakıldı —
// Macbeth dramaturji onayı (Beyti/Filiz) gelmeden işaretleme YAPILMAZ. Adaylar:
// Sahne 3 (Duncan'ı öldürme kararsızlığı), Sahne 5 (Duncan cinayeti — EN HASSAS),
// Sahne 9 (Banquo hayaleti), Sahne 12 (Lady ölümü), Sahne 14 (Macduff sonu).
...
export default function ElYazmasiSayfasi() {
  const { dil } = useDil();
  const s = ceviri(macbethI18n, dil);
  const t = s.elYazmasi;
  const ortak = s.ortak;
  const data = macbethIcerik(dil, karakterGetir('macbeth', dil));
  ...
  // IMZA: OZEL-YAZ-M1 — ozelMi=true ise secilen_dal='ozel' korunur, metin ozel_metin'e gider
    ...
    <main style={{ minHeight: '100vh', backgroundColor: 'var(--bg-base)', color: 'var(--ink)', display: 'flex', flexDirection: 'column' }}>
      ...
      <section style={{ flex: 1, padding: '2.5rem 1.4rem 5rem', maxWidth: 760, margin: '0 auto', width: '100%', display: 'flex', flexDirection: 'column', gap: '2.2rem' }}>
        ...
        <header style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-display), serif', fontStyle: 'italic', fontWeight: 300, fontSize: 'clamp(2.4rem, 6vw, 3.2rem)', color: 'var(--ink)', margin: 0, lineHeight: 1.1 }}>{data.ad || 'Macbeth'}</h1>
...
function YazarinCercevesiSahne({ veri, t }) {
  ...
  // macbeth.js sahnelerWorkbook'tan: olay (sahnede ne oluyor), icsel (ton),
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.55rem', padding: '0.9rem 1rem', background: 'var(--bg-base)', borderLeft: `2px solid ${TON}`, border: '1px solid var(--rule)' }}>
      ...
      {isKarar && Array.isArray(an.secenekler) ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          ...
          {/* IMZA: OZEL-DAL-M1 — "Kendi cevabım": yalnızca travmaDuyarli olmayan çatallarda */}
