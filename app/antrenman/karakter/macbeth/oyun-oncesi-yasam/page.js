// app/antrenman/karakter/macbeth/oyun-oncesi-yasam/page.js
// ITC Actor's Gym — Modül II Macbeth · Oyun Öncesi Yaşam
//
// Workbook s.58-65 birebir karşılığı.
// Sahneye çıkmadan önce ne yaşandı — Macbeth'nin bedeninde taşıdığı geçmiş.
...
export default function OyunOncesiYasamSayfasi() {
  ...
  const oc = ceviri(macbethI18n, dil).oyunOncesi;
  ...
  useEffect(() => {
    async function yukle() {
      const [olaylar, iliskiler] = await Promise.all([
        olayYansimalariniGetir(macbeth.id),
        iliskiYansimalariniGetir(macbeth.id),
      ]);
  ...
  const olaylar = macbeth.oyunOncesi?.olaylar || [];
  const iliskiler = macbeth.oyunOncesi?.iliskiler || [];
      ...
      >
        ...
        <section style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          >
            ...
            <a
              href="/antrenman/karakter/macbeth/timeline"
              style={{
                fontFamily: 'var(--font-body), sans-serif',
                fontWeight: 200,
                fontSize: '0.55rem',
        ...
        <HamletBolumGecisi
          ...
          oncekiYol="/antrenman/karakter/macbeth"
          sonrakiEtiket={oc.gecisSonrakiEtiket}
          sonrakiBaslik={oc.gecisSonrakiBaslik}
          sonrakiYol="/antrenman/karakter/macbeth/timeline"
        />
      </article>
    </main>
  );
