// app/antrenman/karakter/macbeth/senin-cerceven/[no]/page.js
// ITC Actor's Gym — Modül II Macbeth · Senin Çerçeven · Tek Boşluk Detayı
//
...
export default function BoslukDetaySayfasi({ params }) {
  ...
  const sa = ceviri(macbethI18n, dil).seninCerceven.altSayfa;
  ...
  const bosluklar = macbeth.boslukSet || [];
  const bosluk = bosluklar.find((b) => b.no === boslukNo);

  useEffect(() => {
    async function yukle() {
      const [altSorular, genelMetinler, tercihVerisi] = await Promise.all([
        altSoruYansimalariniGetir(macbeth.id),
        boslukGenelMetinleriGetir(macbeth.id),
        tercihleriGetir(macbeth.id),
      ]);
  ...
  useEffect(() => {
    if (!yukleniyor && !bosluk) {
      router.replace('/antrenman/karakter/macbeth/senin-cerceven');
    }
  ...
  function genelMetinDegistir(yeni) {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      const ok = await boslukGenelMetinKaydet(macbeth.id, boslukNo, yeni);
  ...
  const tercihOzetleri = (macbeth.tercihler || []).map((t) => {
    ...
    >
      ...
      >
        ...
        <header style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <a
            href="/antrenman/karakter/macbeth/senin-cerceven"
            style={geriLink}
        ...
        <section style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <CerceveBolumu etiket={`${sa.sonraPrefix}${bosluk.sonraBaslik}`} renk={ALTIN}>
            ...
            {bosluk.sonraSahneNo && (
              <a
                href={`/antrenman/karakter/macbeth/timeline#sahne-${bosluk.sonraSahneNo}`}
                style={{
        ...
        >
          {oncekiNo ? (
            <a
              href={`/antrenman/karakter/macbeth/senin-cerceven/${oncekiNo}`}
              style={navButonStili()}
          ...
          {sonrakiNo ? (
            <a
              href={`/antrenman/karakter/macbeth/senin-cerceven/${sonrakiNo}`}
              style={navButonStili()}
              onMouseEnter={(e) => { e.currentTarget.style.color = 'var(--ink)'; }}
              onMouseLeave={(e) => { e.currentTarget.style.color = 'var(--ink-soft)'; }}
            >
            ...
            <a
              href="/antrenman/karakter/macbeth/senin-cerceven/sentez"
