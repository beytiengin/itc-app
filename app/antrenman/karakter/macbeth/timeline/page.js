// app/antrenman/karakter/macbeth/timeline/page.js
// ITC Actor's Gym — Modül II Macbeth · Timeline · 14 Birim
//
// Generic mimari (A-2): Hamlet ile aynı yapı; fark veride (macbeth.js).
// Macbeth modu: perdeRomen 5 perde id'sidir (I-V: Kehanet · Eylem · Paranoya ·
// Bağımlılık · Çöküş), Macbeth.js perdeTemalari ile bire bir.
// 14 birim, etkileşimli sıcaklık, perde bandı.
...
export default function TimelineSayfasi() {
  ...
  const sahneler = macbeth.sahnelerWorkbook || [];
  const perdeler = macbeth.perdeTemalari || [];

  useEffect(() => {
    async function yukle() {
      const veri = await sahneYansimalariniGetir(macbeth.id);
      setYansimalar(veri);
      setYukleniyor(false);
    }
    yukle();
      ...
      >
        {seciliSahne ? (
          <HamletSahneDetay
            ...
            karakterAd={macbeth.ad}
            tercihler={macbeth.tercihler}
            boslukSet={macbeth.boslukSet}
            kokYol={KOK}
