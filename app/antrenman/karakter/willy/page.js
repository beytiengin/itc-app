...
// ITC Actor's Gym — Willy karakter sayfası
...
export default function WillySayfasi() {
  ...
  useEffect(() => {
    ...
    async function yukle() {
      ...
      if (typeof window !== 'undefined') {
        const goruldu = localStorage.getItem('willy_ilk_giris');
  ...
  function girisKapat() {
    if (typeof window !== 'undefined') {
      localStorage.setItem('willy_ilk_giris', '1');
    }
  ...
  const girisData = ilkGirisMesaji(kalibrasyon, willy.ad);
    ...
    <main style={{ minHeight: '100vh', backgroundColor: '#0a0a0a', color: '#f0ede8', display: 'flex', flexDirection: 'column' }}>
      ...
      <section style={{ flex: 1, padding: '3rem 2rem', maxWidth: '720px', margin: '0 auto', width: '100%', display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', textAlign: 'center' }}>
          ...
          <span style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.6rem', letterSpacing: '0.4em', color: '#c9a96e', textTransform: 'uppercase' }}>{willy.tur}</span>
          <h1 style={{ fontFamily: 'Cormorant Garamond, serif', fontWeight: 300, fontSize: 'clamp(3rem, 8vw, 5rem)', color: '#f0ede8', margin: 0, lineHeight: 1, letterSpacing: '0.05em' }}>{willy.ad}</h1>
          <p style={{ fontFamily: 'Cormorant Garamond, serif', fontStyle: 'italic', fontSize: '1rem', color: '#aaa', margin: 0 }}>{willy.yazar} · {willy.donem}</p>
          <p style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.85rem', color: '#ccc', maxWidth: '500px', lineHeight: 1.8, margin: '1rem auto 0 auto' }}>{willy.ozet}</p>
        </div>
        ...
        <div style={{ padding: '1.2rem 1.5rem', backgroundColor: '#0f0f0f', border: '1px solid #2a2a2a', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          <span style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.55rem', letterSpacing: '0.3em', color: '#c9a96e', textTransform: 'uppercase' }}>Baseline · {willy.baseline.ad}</span>
          <p style={{ fontFamily: 'Cormorant Garamond, serif', fontStyle: 'italic', fontSize: '0.95rem', color: '#ccc', lineHeight: 1.7, margin: 0 }}>{willy.baseline.aciklama}</p>
        </div>
        ...
        {aktifSekme === 'dogrular' && <DogrularBolumu dogrular={willy.dogrular} />}
        {aktifSekme === 'timeline' && <TimelineGorumu sahneler={willy.sahneler} kalibrasyon={kalibrasyon} />}
        {aktifSekme === 'bosluklar' && <BosluklarGorumu karakterId="willy" bosluklar={willy.bosluklar} kalibrasyon={kalibrasyon} />}
        {aktifSekme === 'egzersizler' && <EgzersizListesi karakterId="willy" egzersizler={willy.egzersizler} kalibrasyon={kalibrasyon} onEgzersizTamamlandi={egzersizTamamlandi} />}
