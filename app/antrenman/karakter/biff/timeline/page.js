// app/antrenman/karakter/biff/timeline/page.js
// ITC Actor's Gym — Modül II Biff · Timeline · 11 Birim
//
// Generic mimari (A-2): Hamlet ile aynı yapı; fark veride (biff.js).
// Biff modu: perdeRomen akış-hattı id'sidir (I=Sızıntı, II=Patlama, III=Bedel),
...
export default function TimelineSayfasi() {
  ...
  const sahneler = biff.sahnelerWorkbook || [];
  const perdeler = biff.perdeTemalari || [];

  useEffect(() => {
    async function yukle() {
      const veri = await sahneYansimalariniGetir(biff.id);
      setYansimalar(veri);
      setYukleniyor(false);
    }
    yukle();
      ...
      >
        {seciliSahne ? (
          <HamletSahneDetay
            ...
            karakterId={biff.id}
            karakterAd={biff.ad}
            tercihler={biff.tercihler}
            boslukSet={biff.boslukSet}
            kokYol={KOK}
