// app/antrenman/karakter/biff/el-yazmasi/page.js
...
//   1. Başlık (Modül II · Biff Loman · El yazması)
...
// - Substitution yok: tüm sorgular karakter-merkezli ("Biff bunu yaşarken…").
...
// BIFF SPRINT 3: kayıt anları + hassas/provisional setler boş bırakıldı —
// Biff dramaturji onayı (Beyti/Filiz) gelmeden işaretleme YAPILMAZ. Aday sahneler:
// Sahne 6 (Boston otel — EN HASSAS, kahraman imgesinin yıkılışı),
// Sahne 8 (Bill Oliver/kalem çalma — uyanış), Sahne 11 (restoranı terk),
// Sahne 13 (son yüzleşme — "Ben hiçbir şeyim"), Sahne 14 (Requiem).
...
export default function ElYazmasiSayfasi() {
  const { dil } = useDil();
  const s = ceviri(biffI18n, dil);
  const t = s.elYazmasi;
  const ortak = s.ortak;
  const data = biffIcerik(dil, karakterGetir('biff', dil));
    ...
    <main style={{ minHeight: '100vh', backgroundColor: 'var(--bg-base)', color: 'var(--ink)', display: 'flex', flexDirection: 'column' }}>
      ...
      <section style={{ flex: 1, padding: '2.5rem 1.4rem 5rem', maxWidth: 760, margin: '0 auto', width: '100%', display: 'flex', flexDirection: 'column', gap: '2.2rem' }}>
        ...
        <header style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-display), serif', fontStyle: 'italic', fontWeight: 300, fontSize: 'clamp(2.4rem, 6vw, 3.2rem)', color: 'var(--ink)', margin: 0, lineHeight: 1.1 }}>{data.ad || 'Biff Loman'}</h1>
...
function YazarinCercevesiSahne({ veri, t }) {
  ...
  // biff.js sahnelerWorkbook'tan: olay (sahnede ne oluyor), icsel (ton),
