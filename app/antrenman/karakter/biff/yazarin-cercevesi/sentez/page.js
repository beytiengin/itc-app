// app/antrenman/karakter/biff/yazarin-cercevesi/sentez/page.js
// ITC Actor's Gym — Modül II Biff · Yazarın Çerçevesi · Sentez
//
// "Beş Cümle, Bir Biff" — beş tercihin birleştiği yer.
...
import { biffIcerik } from '../../../../../../data/biff-i18n';
import biffI18n from '../../../../../../data/biff-i18n';
...
export default function SentezSayfasi() {
  const { dil } = useDil();
  const biff = biffIcerik(dil, karakterGetir('biff', dil));
  const ss = ceviri(biffI18n, dil).yazarinCercevesi.sentez;
  const [secimler, setSecimler] = useState({});
  const [yukleniyor, setYukleniyor] = useState(true);
  const [kayitDurumlari, setKayitDurumlari] = useState({});
  const debounceRefs = useRef({});

  const tercihler = biff.tercihler || [];

  useEffect(() => {
    async function yukle() {
      const veri = await tercihleriGetir(biff.id);
      setSecimler(veri);
      setYukleniyor(false);
    }
    yukle();
  ...
  function sentezCumleDegistir(tercihNo, yeni) {
    ...
    debounceRefs.current[tercihNo] = setTimeout(async () => {
      ...
      const ok = await tercihKaydet(biff.id, tercihNo, { sentezCumle: yeni });
    ...
    >
      ...
      >
        <header style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <a
            href="/antrenman/karakter/biff/yazarin-cercevesi"
            style={{
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.9rem', flexWrap: 'wrap' }}>
                  ...
                  <a
                    href={`/antrenman/karakter/biff/yazarin-cercevesi/${tercih.no}`}
                    style={{
                      marginLeft: 'auto',
                      fontFamily: 'var(--font-body), sans-serif',
                      fontWeight: 200,
                ...
                {yapilmis && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', paddingTop: '0.8rem', borderTop: '1px solid var(--bg-elevated)' }}>
                    ...
                    <textarea
                      ...
                      placeholder={dil === 'de' ? `In meinem Biff ${tercih.konu.toLowerCase()} ___, weil ___` : dil === 'en' ? `In my Biff, ${tercih.konu.toLowerCase()} ___, because ___` : `Benim Biff'imde ${tercih.konu.toLowerCase()} ___, çünkü ___`}
                      rows={3}
                      style={{
                        width: '100%',
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                      {sahneNolari.map((no) => (
                        <a
                          key={no}
                          href={`/antrenman/karakter/biff/timeline#sahne-${no}`}
                          style={{
                            fontFamily: 'var(--font-body), sans-serif',
                            fontWeight: 200,
                            fontSize: '0.6rem',
        ...
        <HamletBolumGecisi
          ...
          oncekiYol="/antrenman/karakter/biff/yazarin-cercevesi"
          sonrakiEtiket={ss.gecisSonrakiEtiket}
          sonrakiBaslik={ss.gecisSonrakiBaslik}
          sonrakiYol="/antrenman/karakter/biff/senin-cerceven"
        />
      </article>
    </main>
  );
