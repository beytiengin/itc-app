// app/antrenman/karakter/biff/oyun-oncesi-yasam/page.js
// ITC Actor's Gym — Modül II Biff · Oyun Öncesi Yaşam
//
// Workbook s.58-65 birebir karşılığı.
// Sahneye çıkmadan önce ne yaşandı — Biff'in bedeninde taşıdığı geçmiş.
...
export default function OyunOncesiYasamSayfasi() {
  ...
  const oc = ceviri(biffI18n, dil).oyunOncesi;
  ...
  useEffect(() => {
    async function yukle() {
      const [olaylar, iliskiler] = await Promise.all([
        olayYansimalariniGetir(biff.id),
        iliskiYansimalariniGetir(biff.id),
      ]);
  ...
  const olaylar = biff.oyunOncesi?.olaylar || [];
  const iliskiler = biff.oyunOncesi?.iliskiler || [];
      ...
      >
        ...
        <section style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          >
            ...
            <a
              href="/antrenman/karakter/biff/timeline"
              style={{
                fontFamily: 'var(--font-body), sans-serif',
                fontWeight: 200,
                fontSize: '0.55rem',
        ...
        <HamletBolumGecisi
          ...
          oncekiYol="/antrenman/karakter/biff"
          sonrakiEtiket={oc.gecisSonrakiEtiket}
          sonrakiBaslik={oc.gecisSonrakiBaslik}
          sonrakiYol="/antrenman/karakter/biff/timeline"
        />
      </article>
    </main>
  );
