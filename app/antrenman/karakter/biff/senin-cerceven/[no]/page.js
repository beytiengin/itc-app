// app/antrenman/karakter/biff/senin-cerceven/[no]/page.js
// ITC Actor's Gym — Modül II Biff · Senin Çerçeven · Tek Boşluk Detayı
//
...
import biffI18n, { biffIcerik } from '../../../../../../data/biff-i18n';
...
export default function BoslukDetaySayfasi({ params }) {
  ...
  const sa = ceviri(biffI18n, dil).seninCerceven.altSayfa;
  ...
  const bosluklar = biff.boslukSet || [];
  const bosluk = bosluklar.find((b) => b.no === boslukNo);

  useEffect(() => {
    async function yukle() {
      const [altSorular, genelMetinler, tercihVerisi] = await Promise.all([
        altSoruYansimalariniGetir(biff.id),
        boslukGenelMetinleriGetir(biff.id),
        tercihleriGetir(biff.id),
      ]);
  ...
  useEffect(() => {
    if (!yukleniyor && !bosluk) {
      router.replace('/antrenman/karakter/biff/senin-cerceven');
    }
  ...
  function genelMetinDegistir(yeni) {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      const ok = await boslukGenelMetinKaydet(biff.id, boslukNo, yeni);
  ...
  const tercihOzetleri = (biff.tercihler || []).map((t) => {
    ...
    >
      ...
      >
        ...
        <header style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <a
            href="/antrenman/karakter/biff/senin-cerceven"
            style={geriLink}
        ...
        <section style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <CerceveBolumu etiket={`${sa.sonraPrefix}${bosluk.sonraBaslik}`} renk={ALTIN}>
            ...
            {bosluk.sonraSahneNo && (
              <a
                href={`/antrenman/karakter/biff/timeline#sahne-${bosluk.sonraSahneNo}`}
                style={{
        ...
        >
          {oncekiNo ? (
            <a
              href={`/antrenman/karakter/biff/senin-cerceven/${oncekiNo}`}
              style={navButonStili()}
          ...
          {sonrakiNo ? (
            <a
              href={`/antrenman/karakter/biff/senin-cerceven/${sonrakiNo}`}
              style={navButonStili()}
              onMouseEnter={(e) => { e.currentTarget.style.color = 'var(--ink)'; }}
              onMouseLeave={(e) => { e.currentTarget.style.color = 'var(--ink-soft)'; }}
            >
            ...
            <a
              href="/antrenman/karakter/biff/senin-cerceven/sentez"
