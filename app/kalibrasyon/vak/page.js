...
export default function VAK() {
  ...
  function puanHesapla(c) {
    ...
    const baskın = Object.entries({ V, A, K }).sort((a, b) => b[1] - a[1])[0][0];
    return { V, A, K, baskın };
  }
  ...
  function cevapSec(soruId, puan) {
    ...
    setTimeout(() => {
      if (mevcutSoru < sorular.length - 1) {
        ...
        setSonuc(puanHesapla(yeniCevaplar));
        setAdim('sonuc');
      }
    }, 300);
  }
    ...
    return (
      <main style={{ minHeight: '100vh', backgroundColor: '#0a0a0a', color: '#f0ede8', display: 'flex', flexDirection: 'column' }}>
        ...
        <section style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '4rem 2rem', textAlign: 'center', gap: '2rem' }}>
          ...
          <p style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.9rem', color: '#666', maxWidth: '420px', lineHeight: 1.9, margin: 0 }}>
          ...
          <p style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.75rem', color: '#444', letterSpacing: '0.1em' }}>27 soru · yaklaşık 5 dakika</p>
    ...
    return (
      <main style={{ minHeight: '100vh', backgroundColor: '#0a0a0a', color: '#f0ede8', display: 'flex', flexDirection: 'column' }}>
        ...
        <section style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '3rem 2rem', textAlign: 'center', gap: '3rem' }}>
          <p style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.7rem', letterSpacing: '0.2em', color: '#444', textTransform: 'uppercase', margin: 0 }}>Bu ifade benim için ne kadar doğru?</p>
            ...
            <div style={{ display: 'flex', gap: '0.6rem', justifyContent: 'center' }}>
              {[1,2,3,4,5,6,7].map(puan => (
                <button key={puan} onClick={() => cevapSec(soru.id, puan)}
                  style={{ width: '52px', height: '52px', border: cevaplar[soru.id] === puan ? '1px solid #c9a96e' : '1px solid #2a2a2a', backgroundColor: cevaplar[soru.id] === puan ? '#c9a96e' : 'transparent', color: cevaplar[soru.id] === puan ? '#0a0a0a' : '#666', fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.85rem', cursor: 'pointer', transition: 'all 0.2s ease', flexShrink: 0 }}
  ...
  if (adim === 'sonuc' && sonuc) {
    const bilgi = kanalBilgisi[sonuc.baskın];
    const maksimum = 9 * 7;
    return (
      <main style={{ minHeight: '100vh', backgroundColor: '#0a0a0a', color: '#f0ede8', display: 'flex', flexDirection: 'column' }}>
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '2rem 3rem', borderBottom: '1px solid #1a1a1a' }}>
        ...
        <section style={{ flex: 1, padding: '3rem 2rem', maxWidth: '600px', margin: '0 auto', width: '100%', display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
          <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div>
              <p style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.75rem', letterSpacing: '0.2em', color: '#555', textTransform: 'uppercase', margin: '0 0 0.8rem 0' }}>Baskın Kanalın</p>
          ...
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {[{ kanal: 'V', ad: 'Görsel', puan: sonuc.V }, { kanal: 'A', ad: 'İşitsel', puan: sonuc.A }, { kanal: 'K', ad: 'Kinestetik', puan: sonuc.K }].map(item => (
              <div key={item.kanal} style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <span style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.65rem', letterSpacing: '0.15em', color: item.kanal === sonuc.baskın ? bilgi.renk : '#444', textTransform: 'uppercase', width: '80px', textAlign: 'right', flexShrink: 0 }}>{item.ad}</span>
                <div style={{ flex: 1, height: '1px', backgroundColor: '#1a1a1a', position: 'relative' }}>
                  <div style={{ position: 'absolute', left: 0, top: '-1px', height: '3px', width: `${(item.puan / maksimum) * 100}%`, backgroundColor: item.kanal === sonuc.baskın ? bilgi.renk : '#2a2a2a', transition: 'width 1s ease' }} />
                </div>
                <span style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.65rem', color: '#444', width: '30px', flexShrink: 0 }}>{item.puan}</span>
              </div>
            ))}
          </div>

          <div style={{ borderTop: '1px solid #1a1a1a', paddingTop: '2rem' }}>
            <p style={{ fontFamily: 'Jost, sans-serif', fontWeight: 200, fontSize: '0.85rem', color: '#888', lineHeight: 1.9, margin: 0 }}>{bilgi.aciklama}</p>
          </div>

          <div style={{ backgroundColor: '#111', border: '1px solid #1a1a1a', padding: '1.5rem 2rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', paddingBottom: '3rem' }}>
            ...
            <button onClick={() => { setCevaplar({}); setMevcutSoru(0); setAdim('test'); }}
